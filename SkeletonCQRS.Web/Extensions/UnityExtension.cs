...
namespace SkeletonCQRS.Web.Extensions
{
    public static class UnityExtensions
    {
        public static void RegisterAllTypesForOpenGeneric(this IUnityContainer container, Type openGenericType, params Assembly[] targetAssembly)
        {
            if (!openGenericType.IsGenericTypeDefinition)
                throw new ArgumentException("typeToRegister must be an open generic type", "typeToRegister");

            foreach (Type type in targetAssembly.SelectMany(x => x.GetTypes()))
            {
